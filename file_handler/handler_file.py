...
def del_norm_direct(case_id):
    norm_file_path = os.path.join(MEDIA_ROOT, "case-records", f"case-{case_id}", 'norm')
    shutil.rmtree(norm_file_path)


def del_current_direct(case_id):
    cur_file_path = os.path.join(MEDIA_ROOT, "case-records", f"case-{case_id}", 'current')
...
def add_process_description_img_direct(process_id):
    process_file_path = os.path.join(MEDIA_ROOT, "description", f"process-{process_id}")
    os.makedirs(process_file_path)


def delete_process_description_img_direct(process_id):
    process_file_path = os.path.join(MEDIA_ROOT, "process_images", str(process_id))
